 ...
 */
@Injectable()
export class ChatEventSubscribers {
  private readonly logger: Logger = new Logger('ChatEventSubscribers');
  // one server only listen to one exchange
  // seperate different event by type for different services
  private readonly defaultExchangeName: string = 'onepiece-chat';

  constructor(private readonly userRepository: UserRepository) {
    this.subscribeData('onepiece_chat_queue');
  }
  ...
  execute(event) {
    ...
    this.logger.log(event, 'ChatEventSubscribers');
